...
class GrammarCorrector:
    ...
    def load_model(self):
        ...
        print(f"[GrammarCorrector] Loading {self.model_name} onto {self.device} ({self.dtype})...")
        
        # Use transformers pipeline
        self.pipeline = pipeline(
            "text-generation",
            model=self.model_name,
            device_map="cuda:0" if "cuda" in self.device else "cpu",
            torch_dtype=self.dtype,
        )
        self.is_loaded = True
        print("[GrammarCorrector] Model loaded successfully.")

    def correct(self, text: str, mode: str = "normal") -> str:
        ...
        if mode == "english":
            ...
            system_prompt = "Bạn là trợ lý chỉnh sửa chính tả và dấu câu cho văn bản sau khi nhận diện giọng nói. Hãy sửa lỗi ngữ pháp, thêm dấu câu nếu cần. TUYỆT ĐỐI GIỮ NGUYÊN NGÔN NGỮ GỐC (tiếng Anh giữ nguyên tiếng Anh, tiếng Việt giữ nguyên tiếng Việt), KHÔNG DỊCH. Chỉ xuất ra văn bản sau khi sửa, không giải thích gì thêm."
